...
def LoginView(page, auth,controller):
    ...
    def login_click(e):
        user,msg = auth_controller.login(email_input_value,pass_input.value)
        if user:
            page.session.set("user",user)
            page.go("/dashboard")
        else:
            page.sack_bar=ft.SnackBar(ft.Text(msg))
        ...
        return ft.View("/",[
            ...
            ft.Column([
                ...
                ft.TextButton("Crear una cuenta nueva", on_click=lambda_:page.go("/registro"))
                ],horizontal_aligment=ft.CrossAxisAligment.CENTER,aligment=ft.MainAxisAligment.CENTER)
        ])